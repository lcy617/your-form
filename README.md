# 魔术师 · 一场为你而变的 3D 粒子读心魔术

> 用户进来，网页像一个魔术师，先和你聊天、套话，收集关于你的信息。
> 与此同时，真 3D 空间里的**星点宇宙**已经开始漂浮（透视相机 + 鼠标视差）。
> 酝酿时，粒子从远处球壳**螺旋涌入**；揭晓时，它们**编织成一个专属于你的抽象情绪形态**——
> 一朵绽放、一片星云、一道流瀑、一座晶格、一帘极光、一个漩涡，
> 颜色与能量都来自 AI 对你心境的解读。最后一句台词浮现。
> 2~3 分钟一场的仪式感表演。
...
Vite 打包 → GitHub Pages (前端)
        ...
        Cloudflare Worker (免费代理，藏 API Key)
                ↓
        阿里通义 DashScope (qwen-turbo，负责"读心")
```
...
1. **登场**：打字机浮现神秘台词，问名字；背景已有 ambient 星场漂浮 + 相机缓漂
2. **套话**：4 个问题（心事 / 颜色 / 想看见什么 / 是否信魔术）
3. **酝酿**：屏幕压暗，主粒子云从远处球壳涌入，相机推进
4. **揭晓**：粒子螺旋编织成专属 3D 抽象形态（六大原型之一），相机环绕展示纵深，下方浮现一句话（AI 读心）
5. **落幕**：形态沿径向飞散掠过镜头，相机后拉，告别
...
│   ├── App.vue             # 五幕编排 + 渲染循环 + 相机模式切换
│   ├── style.css           # 黑底 + 多层径向渐变呼吸背景
│   ├── lib/
│   │   ├── config.js       # workerUrl（部署后唯一要改的）
│   │   ├── stage.js        # 打字机 / 输入框 / 转场
│   │   ├── script.js       # 剧本（台词+问题+兜底情绪库）
│   │   └── ai.js           # 两次 AI 调用 + 超时/解析/fallback（输出情绪描述符）
│   └── three/
...
│       └── particles.js        # 主粒子云：涌入/螺旋汇聚/呼吸/消散 + ambient 星场
...
| 想改什么 | 改哪里 |
|---------|--------|
| 台词 / 问题 | `src/lib/script.js` |
| 兜底揭晓形态库 | `src/lib/script.js` 的 `fallbackReveals`（archetype/hue/energy/line） |
